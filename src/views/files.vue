  ...
  <div id="files">
    ...
    >
      ...
      <template #cell(image_upload_time)="data">
        {{
          moment(data.item.image_upload_time)
            .local()
            .format("YYYY-MM-DD HH:mm:ss A")
        }}
      </template>
      <template #cell(image_link)="data">
        <b-link v-bind:href="data.item.image_link" target="_blank">{{
          data.item.image_link
        }}</b-link>
      </template>
...
export default {
  ...
  methods: {
    async infiniteHandler($state) {
      const vm = this;
      axios
        .get(`${vm.$parent.API_BASE_URL}/images`, {
      ...
      vm.$parent.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios
              .delete(
                `${vm.$parent.API_BASE_URL}/delete/${userID}/${imageID}`,
                {
                  headers: {
                    Authorization: `Bearer ${vm.$parent.JWT_TOKEN}`,
                  },
              ...
              .then(function (data) {
                ...
                if (response.data.error) {
                  ...
                  vm.$parent.$toast.success("Successfully deleted the file.", {
                    position: "top-right",
                  });
                  console.log(vm.items);
                  const index = vm.items.findIndex(
                    (x) => x.image_id === `${imageID}`
                  );
                  console.log(index);
